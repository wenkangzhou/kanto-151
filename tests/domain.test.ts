...
test('chapter advancement ignores evolutions, capture pools exclude duplicates and legends', () => {
  ...
  assert.deepEqual(capturePool(emptySnapshot).map(p => p.id), first.pokemonIds);
...
test('Eevee has three independent Kanto evolutions and originals remain eligible', () => {
  ...
  assert.equal(collectionState(pokemonById.get(4)!, emptySnapshot), 'available');
...
test('legendary thresholds and the ticket-free final Mew encounter', () => {
  ...
  assert.equal(legendaryEligible(150, first(140)), false); // Dratini chapter incomplete.
  assert.equal(legendaryEligible(150, snapshotWith([...Array.from({ length: 139 }, (_, i) => i + 1), 147])), true);
