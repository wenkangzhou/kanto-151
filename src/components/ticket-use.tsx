...
export function TicketUse({ preview }: { preview?: { snapshot: CollectionSnapshot; tickets: InventoryTicket[]; onUse: (target: Pokemon) => void } } = {}) {
  ...
  return <section ref={section} className="detail-panel ticket-journey" id="use-ticket">
    ...
    {activeStep === 1 && <><h3>点一张，看看里面的惊喜</h3><div className="picture-tickets" role="group" aria-label="选择奖励券">{tickets.map(item => <button type="button" key={item.id} aria-pressed={ticketId === item.id} className={`picture-ticket ${item.type} ${ticketId === item.id ? 'chosen' : ''}`} onClick={() => { setTicketId(item.id); setTargetId(null); setError(''); setRetrying(false); goToStep(2); }}><span className="ticket-check">{ticketId === item.id ? <Check size={20} /> : <Ticket size={20} />}</span>{item.type === 'evolution' ? <Sparkles size={46} /> : <Mountain size={46} />}<strong>{item.type === 'evolution' ? '进化券' : '传说券'}</strong><span className="picture-ticket-reason">{item.reason || '一次值得记住的努力'}</span><small>{new Date(item.createdAt).toLocaleDateString('zh-CN')}</small></button>)}</div></>}
    {activeStep === 2 && ticket && <><h3>{ticket.type === 'evolution' ? '想看看谁的新模样？' : '想和哪位神秘伙伴相遇？'}</h3>{options.length ? <><div className="picture-partners" role="group" aria-label="选择新伙伴">{options.map(target => <div className={`picture-partner ${selected?.id === target.id ? 'chosen' : ''}`} key={target.id}><button type="button" className="picture-partner-choice" aria-pressed={selected?.id === target.id} onClick={() => { setTargetId(target.id); goToStep(3); }}><span className="partner-check">{selected?.id === target.id ? <Check size={22} /> : <Sparkles size={20} />}</span><Transformation target={target} /></button><span className="partner-type-pictures">{target.types.map(type => <TypePicture type={type} key={type} />)}</span></div>)}</div>{ticket.type === 'evolution' && <p className="form-note">原来的伙伴会留下，新模样也会加入图鉴。</p>}</> : <div className="ticket-wait"><LockKeyhole size={38} /><h4>惊喜先留在背包里</h4><p>{ticket.type === 'evolution' ? '再认识一些伙伴，就有机会发现新的模样。' : '再收集一些伙伴，远方的传说就会开启。'}这张券没有被使用。</p></div>}<div className="journey-actions"><button className="button secondary" onClick={() => goToStep(1)}><ArrowLeft size={19} /> 换张券</button></div></>}
