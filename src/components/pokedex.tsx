...
export function Pokedex() {
  ...
  const results = useMemo(() => pokemon.filter(p => {
    ...
    const matchesState = filter === 'all' || (filter === 'collected' ? known : filter === 'locked' ? !known : state === 'evolvable' || state === 'evolution-ready');
  ...
  return <div className="page pokedex-page">
    ...
    <div className="results-meta"><span aria-live="polite">{results.length} 位伙伴{type !== 'all' && ' · 仅显示已发现的属性'}</span><div className="legend"><span><CollectionMark state="available" /> 可遇见</span><span><CollectionMark state="collected" /> 已收集</span><span><CollectionMark state="evolvable" /> 可进化</span><span><CollectionMark state="needs-evolution-ticket" /> 需要进化券</span><span><CollectionMark state="locked" /> 未发现</span></div></div>
