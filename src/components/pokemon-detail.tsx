...
import { companionNeighbors, swipeDirection, detailReturn, detailHref, teamNeighbors } from '@/domain/navigation';
import { motion } from 'framer-motion';
import { ArrowRight, ChevronLeft, ChevronRight, BookOpen, Sparkles, Heart, ChartNoAxesColumnIncreasing, CalendarDays } from 'lucide-react';
...
import { PartnerGreeting } from './partner-greeting';
import { PokemonArt } from './pokemon-art';
import { ReadAloud } from './read-aloud';
import { TypeBadge } from './type-badge';
import { SizeComparison } from './size-comparison';
import { useTeamView } from './team-memory';
import { centerPartners } from '@/domain/center';
export function PokemonDetail({ pokemon: p }: { pokemon: Pokemon }) {
  const { snapshot } = useCollection();
  const router = useRouter();
  const search = useSearchParams();
  const from = search.get('from');
  const receipt = search.get('receipt');
  const back = detailReturn(from, receipt);
  const href = (id: number) => detailHref(id, from, receipt);
  const gesture = useRef<{ x: number; y: number; pointer: number } | null>(null);
  const suppressClick = useRef(false);
  const teamView = useTeamView();
  const team = snapshot.team ?? [];
  const teamScope = team.includes(p.id) ? team : centerPartners(snapshot.records, team, teamView.filter, teamView.sort).map(record => record.pokemonId);
  const neighbors = from === 'team' ? teamNeighbors(p.id, teamScope) : companionNeighbors(p.id, snapshot.records.map(record => record.pokemonId));
  ...
  return <div className="page detail-page"><div className="detail-navigation"><Link href={back.href} className="back-link"><BookOpen size={25} /> {back.label}</Link><div>{neighbors.previous !== null && <Link scroll={false} replace href={href(neighbors.previous)} aria-label="上一只宝可梦"><ChevronLeft size={20} /></Link>}<span className="mono">{dexNumber(p.id)}</span>{neighbors.next !== null && <Link scroll={false} replace href={href(neighbors.next)} aria-label="下一只宝可梦"><ChevronRight size={20} /></Link>}</div></div>
    <section className={`detail-hero detail-${known ? p.types[0] : 'locked'}`}><div className="detail-image detail-swipe" onPointerDown={event => {
      ...
      if (id !== null) router.replace(href(id), { scroll: false });
    }} onPointerCancel={() => { gesture.current = null; }} onClickCapture={event => {
      if (suppressClick.current) { event.preventDefault(); event.stopPropagation(); suppressClick.current = false; }
    }}><span className="detail-big-number mono" aria-hidden>{String(p.id).padStart(3, '0')}</span><motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }} key={p.id}>{known ? <PartnerGreeting pokemon={p} /> : <PokemonArt pokemon={p} hidden priority />}</motion.div><span className="detail-image-caption mono">KANTO FIELD NOTES / {String(p.id).padStart(3, '0')}</span></div><div className="detail-intro"><span className="eyebrow">{dexNumber(p.id)} · {stateNames[state]}</span><h1>{known ? p.name : '神秘的伙伴'}{known && <ReadAloud text={p.name} label={`听听${p.name}的名字`} />}</h1><div className="detail-english">{known ? p.englishName : 'A FRIEND YET TO MEET'}</div>{known ? <><p className="traditional-name">繁体中文 · {p.traditionalName}</p><div className="detail-types">{p.types.map(type => <TypeBadge type={type} key={type} />)}</div><p className="pokemon-description">{p.description}<ReadAloud text={`${p.name}。${p.description}`} /></p><SizeComparison pokemon={p} /></> : <div className="locked-description"><CollectionMark state={state} /><p>{unlockHint(p, snapshot)}</p><span>相遇之后，名字和秘密就会出现在这里。</span></div>}</div></section>
    ...
    <section className="acquisition-note"><div className="note-icon"><Heart size={23} /></div><div><span className="eyebrow">OUR LITTLE MEMORY</span><h2>我们的相遇，是因为……</h2><p>「{record?.reason || '一次值得珍藏的成长'}」</p><span className="memory-date"><CalendarDays size={14} />{record ? new Date(record.acquiredAt).toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai' }) : ''} · {record?.method === 'evolution' ? '进化相遇' : '冒险相遇'}{snapshot.source === 'demo' ? ' · 示例记录' : ''}</span></div></section><Link href={back.href} className="button detail-dex-return"><BookOpen size={29} />{back.label}<ArrowRight size={23} /></Link></> : <div className="mystery-note"><Sparkles size={20} /><h2>把惊喜，留给相遇的那一天</h2><p>继续认识新伙伴吧，这一页正在等你写下故事。</p><Link href={back.href} className="button secondary">{back.label} <ArrowRight size={16} /></Link></div>}
  </div>;
}
