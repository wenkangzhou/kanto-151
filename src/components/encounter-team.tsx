...
export function EncounterTeam({ partner, previousId, disabled, preview = false }: { partner: Pokemon; previousId?: number | null; disabled: boolean; preview?: boolean }) {
  ...
  if (!preview && team.includes(partner.id)) return <div className="encounter-team"><p className="encounter-team-saved" role="status"><Check size={20} />已经在小队里，一起出发吧！</p><Link href="/team" className="button secondary"><UsersRound size={24} />去小队<ArrowRight size={20} /></Link></div>;
...
export function InviteDialog({ partner, previousId, expected, onClose, onDone }: { partner: Pokemon; previousId?: number | null; expected: number[]; onClose: () => void; onDone?: () => void }) {
  ...
  return <dialog ref={dialog} className="team-dialog invite-dialog" aria-labelledby="invite-title" onCancel={event => { event.preventDefault(); if (!saving.current) onClose(); }}>
    ...
    </div><div className="team-dialog-footer">{error && <p className="team-error" role="alert">{error}</p>}{!failed && <button className="button" disabled={busy || slot === null} onClick={() => void confirm()}>{busy ? '正在安排…' : leaving ? '确认交换' : '一起出发'}<ArrowRight size={20} /></button>}</div>
  </dialog>;
}
