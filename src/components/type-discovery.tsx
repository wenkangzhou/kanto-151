...
import { ArrowRight, Shield, Sparkles, ChevronDown, Zap, Swords, ShieldAlert } from 'lucide-react';
...
export function TypeDiscovery({ partner, discovered }: { partner: Pokemon; discovered: Set<number> }) {
  ...
  return <section className="detail-panel type-discovery"><div className="section-heading"><h2><Sparkles size={23} /> 看图认识属性</h2></div>
    <div className="type-lessons"><section className="type-lesson outgoing"><h3><Swords size={25} aria-hidden="true" /> 我出招！<ReadAloud text={targets.length ? `${TYPE_NAMES[attack]}招式！攻击${targets.map(type => TYPE_NAMES[type]).join('、')}属性时，力量会变成两倍。${partner.types.length > 1 ? '点另一个属性，可以换招式。' : ''}` : '一般招式也能攻击，只是没有特别擅长的属性对手。'} /></h3>
