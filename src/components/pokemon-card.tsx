...
export function PokemonCard({ pokemon: p, priority = false }: { pokemon: Pokemon; priority?: boolean }) {
  ...
  return <div className={`pokemon-card card-${known ? p.types[0] : 'locked'} state-${state}`}><Link className="pokemon-card-link" href={`/pokemon/${p.id}${pathname === '/' ? '?from=home' : ''}`} aria-label={`${dexNumber(p.id)} ${known ? p.name : '未发现的宝可梦'}，${stateNames[state]}`}>
