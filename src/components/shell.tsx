...
export function Shell({ children, musicAvailable = false }: { children: React.ReactNode; musicAvailable?: boolean }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="journal-note"><Sparkles size={20} /><p>每一份小小的努力，<br />都值得被记住。</p><span className="mono">ONE STEP, ONE DISCOVERY.</span></div><Link href="/parent" className="parent-link"><ShieldCheck size={18} /> 家长中心 <ArrowUpRight size={15} /></Link></div>
    </aside>
    <div className="main-shell"><header className="topbar"><Link className="mobile-brand" href="/">KANTO <b>151</b></Link><span className="desktop-breadcrumb">我的冒险手帐 <span>/</span> 关都地区</span><div className="topbar-actions"><BackgroundMusic available={musicAvailable} /><PwaControls /><Link className="mobile-parent-button" href="/parent"><ShieldCheck size={18} aria-hidden="true" /><span>家长中心</span></Link>{!live && <button className="preview-toggle" onClick={toggleDemo} title="切换示例收藏与全新图鉴"><span className={`status-dot ${demo ? '' : 'empty'}`} />{demo ? '示例冒险' : '全新图鉴'}<span className="toggle-label">切换</span></button>}</div></header>
