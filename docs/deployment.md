...
小队陪伴流程：主页每 8 秒轮换一位随行伙伴，可暂停；系统开启减少动态效果时不自动轮换。无队伍时仍展示皮卡丘向导。捕捉、传说相遇和进化结果完成后可邀请入队，满员必须选择交换对象并确认。进化前形态在队伍中时默认选中其位置，有空位时也可选用空位。取消入队不会消耗奖励或改变收藏；已入队的结果再次打开会显示已在队伍中。此功能沿用 004，无新增迁移。
