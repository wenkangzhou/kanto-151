...
`/team` 展示最多六位随行伙伴及其余所有已收集伙伴。`kanto_team` 以孩子和位置为主键，位置限制 1—6，同一宝可梦不得重复；复合外键限制为该孩子已收集的宝可梦。精灵中心从「收藏减小队」派生，不复制或移动收藏记录。进化保留原形态的现有规则不变，新形态先进入中心。

`POST /api/team` 从设备会话取得孩子 ID，不需要家长 PIN。`kanto_set_team` 锁定孩子记录，校验六人上限、所有权与调用者看到的旧队伍，再原子更新；相同结果可安全重试，旧设备的冲突修改被拒绝。响应包含最新快照，前端阻止重复提交并使更早的刷新响应失效。页面回到前台时刷新家庭数据；没有实时订阅。表开启 RLS，匿名及 authenticated 角色没有表权限或函数执行权限。
