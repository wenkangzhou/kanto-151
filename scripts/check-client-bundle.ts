...
if (config) {
  const values = [...new Set([process.env.SUPABASE_URL?.trim(), config.url, config.publishableKey, config.secretKey].filter((value): value is string => Boolean(value)))];
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) scan(path, extensions);
        ...
        if (variants.some(value => content.includes(value))) {
          throw new Error('构建检查失败：公开产物包含 Supabase 配置值，请检查服务端与客户端边界。');
        }
      }
    }
  };
  ...
  console.log(`公开构建产物检查通过：${count} 个文件未包含 Supabase 配置值。`);
} else {
